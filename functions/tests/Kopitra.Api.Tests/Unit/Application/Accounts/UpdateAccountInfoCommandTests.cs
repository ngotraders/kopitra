...
public class UpdateAccountInfoCommandTests
{
    ...
    public async Task UpdateAccountInfo_WithAllProperties_UpdatesSuccessfully()
    {
        ...
        Assert.AreEqual("new-key", account.ApiKey);
        Assert.AreEqual("new-secret", account.ApiSecret);
    }
    ...
    [TestMethod]
    public async Task UpdateAccountInfo_WithOnlyApiCredentials_UpdatesSuccessfully()
    {
        // Arrange
        var accountId = new AccountId($"account-{Guid.NewGuid()}");
        var userId = new UserId($"user-{Guid.NewGuid()}");
        ...
        Assert.IsNotNull(account);
        Assert.AreEqual("updated-key", account.ApiKey);
        Assert.AreEqual("updated-secret", account.ApiSecret);
    }
}
